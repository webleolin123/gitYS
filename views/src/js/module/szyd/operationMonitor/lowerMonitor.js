    ...
    function () {
        ...
        var eventHandle = {
            ...
            reportTableLine: function (objSet) {
                ...
                if (!reportTableSet) {
                    table.render(common.tableInitParams({
                        elem: '#reportTable',
                        url: baseUrl + 'sasmeetingmgr/statistics/selectSynctIndexTable',
                        method: 'post',
                        height: '620',
                        toolbar: '',
                        cols: [
                            [
                                ...
                                {field: 'smName', title: '企业', sort: true},
                                {field: 'smIcon', title: '上报时间', sort: true},
                                {field: 'regulationcount', title: '决策制度', sort: true},
                                {field: 'itemcount', title: '事项清单', sort: true},
                                {field: 'meetingcount', title: '决策会议', sort: true},
                                {field: 'subjectcount', title: '审议议题', sort: true},
                                {field: 'executioncount', title: '组织实施', sort: true},
                                {field: 'exceptioncount', title: '异常情况', sort: true},
                                {field: 'ycqksjyts', title: '整改率', sort: true}
                            ]
                        ],
                        where: {
            ...
            reportCardLine: function (objSet) {
                ...
                function pageDataSet(data) {
                    ...
                    if (data.length > 0) {
                        $.map(data, function (item) {
                            var statusHtml = '';
                            if (!!item.reportStatus && item.reportStatus === "yes") {
                                statusHtml = '<div class="status status-success">已上报</div>';
                            } else if (!item.reportStatus || item.reportStatus === "no") {
                            ...
                            html += '<li>\n' +
                                ...
                                '                       </li>\n' +
                                '                       <li>\n' +
                                '                           <span class="warning number ">-</span>\n' +
                ...
                var dataLength;
                common.fetchPost('sasmeetingmgr/statistics/selectSynctIndexTable', {
                    reportStatus: (objSet && objSet.reportStatus) ? objSet.reportStatus : 'all',
            ...
            totalSet: function () {
                ...
                common.fetchPost('sasmeetingmgr/statistics/selectIndexTableAllCount', {
                    ...
                    if (data) {
                        ...
                        // $('.ycqksjytsum').text(data.ycqksjytsum);
                    }
