/**
 ...
 异步调用数据返回格式 toOpen 为真自动展开节点
 异步调用数据返回格式 isParent 是否为父节点
...
layui.config({}).extend({}).define(['laytpl', 'laypage', 'layer', 'form'], function(exports) {
	...
	var $ = layui.$,
		...
		table = {
			config: { //全局配置项,表格级别
				...
				cols: { //节点级别的附加字段
					...
					,
					isParent: false //是否为父节点
					,
					toOpen: false   //是否自动展开节点
				},
				initWidth: { //默认列宽度定义
					checkbox: 48,
					space: 15,
			...
			init: function(filter, settings) {
				...
				//添加额外style
				var css = '.layui-table-cell-hover{background-color: #d6d6d6;}',
			...
			radioStatus: function(tableId) {
				...
				for (var i= 0;i<data.length;i++) {
					if (data[i][table.config.indexName] == v) {
						retObj = data[i];
					};
				}
			...
			delRow: function(tableId, data) {
				...
				for (var i= 0;i<delDatas.length;i++) {
					var uo = that.treeFindUpData(delDatas[i]);
					if (uo) {
						upDelDatas.push(uo);
					}
				}
				...
				for (var i= 0;i<sonList.length;i++) {
					var index = sonList[i][table.config.indexName];
				...
				for (var i = 0, len = list.length; i < len; i++) {
					...
					for (var j= 0;j<sonList.length;j++) {
						if (sonList[j][table.config.indexName] === list[i][table.config.indexName]) {
							isP = false;
						}
					}
					if (isP) {
				...
				//处理父级节点
				for (var i= 0;i<upDelDatas.length;i++) {
					that.renderTreeConvertShowName(upDelDatas[i]);
				}
			...
			treeOpenAll: function(tableId, isOpen) {
				...
				for (var i= 0;i<list.length;i++) {
					that.treeNodeOpen(list[i], isOpen);
				}
			}
			/**
			 * 获取全部需要子节点对象集合
			...
			treeFindSonList: function(tableId, data) {
				...
				for (var i= 0;i<delDatas.length;i++) {
					if (delDatas[i].children.length > 0) {
						var temSonList = that.treeFindSonData(delDatas[i]);
						for (var j =0 ; j<temSonList.length;j++) {
							if (!delIds[temSonList[j][table.config.indexName]]) {
								sonList.push(temSonList[j]);
								delIds[temSonList[j][table.config.indexName]] = temSonList[j][table.config.indexName];
							}
						}
					}
					sonList.push(delDatas[i]);
					delIds[delDatas[i][table.config.indexName]] = delDatas[i][table.config.indexName];
				}
				return sonList;
			},
			...
			treeFindSonIdFields: function(tableId, data) {
				...
				var sonList = table.treeFindSonList(tableId, data);
				for (var i =0; i<sonList.length;i++) {
					idField.push(sonList[i][that.config.idField]);
				}
				return idField;
			}
			/**
			...
			treeFindRowCheck: function(tableId) {
				...
				if (list) {
					for (var i = 0; i<list.length;i++) {
						if (list[i][table.config.cols.isRowCheck]) {
							rowchecks.push(list[i]);
						}
					}
				}
				return rowchecks;
			 ...
			 */
			,
			treeRowCheck: function(tableId, index,field) {
				var that = table.getClass(tableId);
				if (!that) return;
				...
				if (that.config.model == "tree") {
					for (var i=0;i<list.length;i++) {
						list[i][table.config.cols.isRowCheck] = false;
					}
					o[table.config.cols.isRowCheck] = true;
					that.layBody.find("tr").css("background-color", "");
					that.getTr(index).css("background-color", "#ccc");
				}
				if (typeof that.config.onClickCell === 'function' ) that.config.onClickCell(index, o,field)
				else (typeof that.config.onClickRow === 'function' && that.config.onClickRow(index, o));
			 ...
			 */
			,
			treeHeadDbClick: function (tableId, field) {
				var that = table.getClass(tableId),
					title = $("." + tableId + " thead [data-field=" + field + "] span").html();
				typeof that.config.headDbClick === 'function' && that.config.headDbClick(field,title);
			}
			/**
			 * 工具方法对象
			 */
			...
			kit: {
				...
				restNumbers: function(list) {
					...
					for (var j = 0 ;j<list.length;j++) {
						list[j][table.config.indexName] = i;
						i++;
					}
				}
			}
		...
		ELEM_CELL_HOVER = 'layui-table-cell-hover',
		...
		TPL_HEADER = function(options) {
			...
			return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" '
				...
				,
				'<th data-field="{{ item2.field||i2 }}" class="{{item2.classs||""}}" {{# if(item1.length==i2+1){ }}style="border-right-width: 0px;"{{# } }} {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} ' +
			...
			if (!map[temo[field_upId]]) { //没有找到父节点
				if (!rootMap[temo[field_upId]]) { //还不存在
					...
					for (var j = 0; j < rootList.length; j++) {
						if (rootList[j] === temo[field_upId]) temis = false;
					}
	...
	Class.prototype.resetDataTreeList = function (list, rootList) {
		...
		for (var i = 0; i < rootList.length; i++) {
			var temTreeObj = fa(rootList[i]); //递归
			if (temTreeObj) {
				for (var j = 0; j < temTreeObj.length; j++) {
					treeList.push(temTreeObj[j]);
				}
			}
		}
		return treeList;
	...
	Class.prototype.resetDataTableList = function (treeList) {
		...
		for (var i = 0; i < tableList.length; i++) {
			var uo = that.treeFindUpData(tableList[i]);
			if (!uo || (uo[table.config.cols.isOpen] && uo[table.config.cols.isShow])) { //没有父亲：显示；父亲打开状态（显示状态：显示；隐藏状态：隐藏）
				tableList[i][table.config.cols.isShow] = true;
			} else {
				tableList[i][table.config.cols.isShow] = false;
			}
		}
		return tableList;
	};
		...
		function f(o) {
			if (o.children.length > 0) {
				for (var i =0;i<o.children.length ;i++) {
					objs.push(o.children[i]);
					if (o.children[i].children.length > 0) {
						f(o.children[i]);
					}
				}
			}
	...
	Class.prototype.treeConvertShowName = function(o) {
		var that = this,
			options = that.config;
		var isTreeNode = (o.children && o.children.length > 0)||o.isParent;
	...
	Class.prototype.treeNodeOpen = function (o, isOpen) {
		var that = this,
			...
			dbClickI = tr.find('.layui-tree-head'),
			flag = false,
			response = options.response;

		if (o.isParent && o.children && options.async.enable && o.children.length <= 0 && !o.isLoad) {
			if (dbClickI.data("click")) {
				return;
			}
			flag = true;
			dbClickI.html('&#xe63d;');
			dbClickI.data("click", true);
			var params = {};
			var autoParam = options.async.autoParam;
			for (var i = 0; i < autoParam.length; i++) {
				params[autoParam[i]] = o[autoParam[i]];
			}
			...
			$.ajax({
				...
				success: function (res) {
					...
					var currentLen = datas.length;

					for (var q = 0; q < datas.length; q++) {
						if (datas[q][options.idField] == o[options.idField]) {
							// o.children = data;
							for (var j = data.length; j > 0; j--) {
								table.addRow(that.key, q + 1, data[j - 1]);
							}
							break;
						}
					}
	...
	Class.prototype.pullData = function(curr, loadIndex) {
		...
		if (options.url) { //Ajax请求
			...
			$.ajax({
				type: options.method || 'get',
				url: options.async.enable?options.async.url:options.url,
				data: data,
				dataType: 'json',
				contentType: options.contentType,
				headers: options.headers,
				success: function(res) {
					...
					that.renderRowCheck();
					layui.each(res[response.dataName],function (i,o) {
						if (o.toOpen) {
							that.treeNodeOpen(o, true);
						}
	...
	Class.prototype.renderTd = function (obj, cols, numbers, i3) {
		...
		var td = ['<td data-field="' + field + '" ' + function () {
			...
			if (cols.classs) attr.push('class="' + cols.classs + '"'); //单元格最小宽度
			return attr.join(' ');
		}() + '>'
	...
	Class.prototype.renderRowCheck = function() {
		...
		if (list) {
			for (var i = 0 ; i<list.length;i++) {
				var tr = that.layBody.find('tr[data-index=' + list[i][table.config.indexName] + ']');
				if (list[i][table.config.cols.isRowCheck]) {
	...
	Class.prototype.tableHeight = function() {
		...
		//确定高度
		if (!table.kit.isNumber(optionsFirst.height)&&options.height) { //如果非固定高度，则计算高度
			var htremove = 0; //减去的高度
			if (options.heightRemove && table.kit.isArray(options.heightRemove)) {
				var htatt = options.heightRemove;
				for (var i = 0;i<htatt.length;i++) {
					var temh = table.kit.isNumber(htatt[i]) ? htatt[i] : $(htatt[i]).outerHeight(true);
	...
	Class.prototype.events = function() {
		var that = this,
			...
			bodytd = that.layBody.find('td'),
			resizing;
		//行点击事件
		if (!typeof that.config.onClickCell === 'function')
		...
		//单元格点击事件
			...
			bodytd.unbind('click').on('click', function(e) {
				var index = $(this).parent().attr("data-index");
				var field = $(this).attr("data-field");
				table.treeRowCheck(that.config.id, index,field);
			});
		//行双击事件
		...
		//表头双击
		th.unbind('dblclick').on('dblclick', function(e) {
			var field = $(this).attr("data-field");
			table.treeHeadDbClick(that.config.id, field);
		});
		//拖拽调整宽度
	...
	Class.prototype.eventsinit = function () {
		...
		if (typeof that.config.onClickCell === 'function') {
			...
			that.layBody.on('mouseenter', 'td', function () {
				var othis = $(this),
					index = othis.index();
				// that.layBody.find('tr:eq(' + index + ')').addClass(ELEM_CELL_HOVER)
				// othis.addClass(ELEM_CELL_HOVER)
			});
			that.layBody.on('mouseleave', 'td', function () {
				var othis = $(this),
					index = othis.index();
				// that.layBody.find('tr:eq(' + index + ')').removeClass(ELEM_CELL_HOVER)
				// othis.removeClass(ELEM_CELL_HOVER)
			});
			...
			switch (events) {
				case 'LAYTABLE_EXPORT': //导出
					...
					for (var i in options.where) {
						where += i + "=" + options.where[i] + "&";
					}
					window.open(options.excelUrl + "?" + where.substring(0, where.length - 1));
					break;
		...
		that.elem.on('click', 'input[name="' + TABLE_CHECKBOX_ID + '"]+', function () {
			...
			if (isAll) {
				var list = table.getDataList(that.key);
				for (var i = 0; i < list.length; i++) {
					if (!list[i][table.config.cols.cheDisabled]) { //可以选择的才设置
						that.setCheckData(list[i][table.config.indexName], checked);
					}
				}
			} else {
				...
				if (options.isTree) {
					...
					for (var i = 0; i < sonList.length; i++) {
						if (!sonList[i][table.config.cols.cheDisabled]) { //可以选择的才设置
							that.setCheckData(sonList[i][table.config.indexName], checked);
						}
					}
					...
					var temf = function (o) {
						...
						if (o && o.children.length > 0) {
							var temis = true;
							for (var i = 0; i < o.children.length; i++) {
								if (!o.children[i][table.config.cols.isCheckName]) {
									temis = false;
								}
							}
							if (temis) {
