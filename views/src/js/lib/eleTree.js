...
layui.define(["jquery","laytpl"], function (exports) {
    ...
    //模板渲染方法重新分装
    var tpl_elem=function (options,floor,parentStatus,getData) {

        return [
            '{{# for(var i=0;i<d.length;i++){ }}',
            '<div class="eleTree-node" data-'+options.request.extendAttr+'="{{d[i]["'+options.request.extendAttr+'"]}}"  data-'+options.request.key+'="{{d[i]["'+options.request.key+'"]}}" eletree-floor="'+floor+'" style="display: none;">',
            ...
            function() {
                if(options.showIcon){
                    return [
                        '{{# if(d[i]["'+options.request.extendAttr+'"]) { }}',
                        '<i style="padding-right: 3px;" class="icon iconfont icon-{{d[i]["' + options.request.extendAttr + '"]}}"></i>',
                        '{{# } }}',
                    ].join("");
                }
                return ''
    ...
    Class.prototype={
        ...
        render: function() {
            ...
            // load加载框
            options.elem.append('<div class="eleTree-loadData" style="text-align: center"><i style="margin-top: 30px;" class="layui-icon layui-icon-loading layui-icon layui-anim layui-anim-rotate layui-anim-loop"></i></div>');
        ...
        renderData: function() {
            ...
            // 渲染第一层
            laytpl(tpl_elem(options,0,null,options.data)).render(options.data, function(string){
        ...
        ajaxGetData: function() {
            ...
            var data = $.extend({}, options.where); //请求带的参数
            ...
            $.ajax({
                ...
                ,success: function(res){
                    if(!res[options.response.dataName]){
                    ...
                    if(options.lazy && el.hasClass("lazy-icon")){
                        ...
                        options.load(d,function(getData) {
                            ...
                            if(d[options.request.children] && d[options.request.children].length>0){
                                // 只渲染获取到的数据
                                laytpl(tpl_elem(options,floor,eletreeStatus,getData)).render(getData, function(string){
                        ...
                        var eletreeStatus=eleTreeNodeContent.children("input.eleTree-hideen").attr("eletree-status");
                        if (d) {
                            d[options.request.children] && d[options.request.children].length > 0 && laytpl(tpl_elem(options, floor, eletreeStatus, d[options.request.children])).render(d[options.request.children], function (string) {
            ...
            if(data.length>0){
                $.map(data,function(val,index) {
                    ...
                    if(val[options.request.children] && val[options.request.children].length>0){
                        ...
                        laytpl(tpl_elem(options,floor,null,val[options.request.children])).render(val[options.request.children], function(string){
                            el.append(string);
                            // 判断是否展开所有
        ...
        keySearchToOpera: function(key,callback) {
            ...
            var fn=function(data) {
                ...
                for(;obj.i<obj.len;obj.i++){
                    if (data[obj.i] && options.request.key && data[obj.i][options.request.key] && data[obj.i][options.request.key] != key) {
        ...
        updateKeyChildren: function(key,data) {
            ...
            this.keySearchToOpera(key,function(d,obj) {
                ...
                // dom更新
                node.length!==0 && laytpl(tpl_elem(options,floor,null,data)).render(data, function(string){
        ...
        updateKeySelf: function(key,data) {
            ...
            var floor=Number(node.attr("eletree-floor"))+1;
            data[options.request.name] && node.children(".eleTree-node-content-label").text(data[options.request.name]).attr('title',data[options.request.name]);
        ...
        append: function(key,data) {
            ...
            if(key===null || key===""){
                ...
                laytpl(tpl_elem(options,0,"0",[data])).render([data], function(string){
            ...
            this.keySearchToOpera(key,function(d,obj) {
                ...
                if(!isExist){
                    ...
                    node.length!==0 && laytpl(tpl_elem(options,floor,eletreeStatus,[arr[len-1]])).render([arr[len-1]], function(string){
        ...
        insertBefore: function(key,data) {
            ...
            this.keySearchToOpera(key,function(d,obj) {
                ...
                eletreeStatus=eletreeStatus==="2" ? "0" : eletreeStatus;
                node.length!==0 && laytpl(tpl_elem(options,floor,eletreeStatus,[data])).render([data], function(string){
        ...
        getChecked: function(leafOnly, includeHalfChecked) {
            ...
            el.each(function(index,item) {
                ...
                // var extendAttr=$(item).parent(".eleTree-node-content").parent(".eleTree-node").data(options.request.extendAttr);
                var extendAttr=$(item).parent(".eleTree-node-content").parent(".eleTree-node").attr('data-extendattr');
                if(!!extendAttr && extendAttr!=null && extendAttr!='null'){
                    obj[options.request.extendAttr]=extendAttr;
                }else{
                    obj[options.request.extendAttr]='';
                }
                                ...
                                label.siblings(".eleTree-node-content-input").focus().off().on("blur",function() {
                                    ...
                                    layui.event.call(node, MOD_NAME, 'node'+val+'('+ _self.filter +')', {
                                        ...
                                        setData: function(o) {
                                            ...
                                            // 修改dom
                                            $(inpThis).siblings(".eleTree-node-content-label").text(newObj[options.request.name]).attr('title',newObj[options.request.name]).show();
                                    ...
                                    if($.inArray("add.async",options.contextmenuList)===-1){
                                        ...
                                        // 修改dom
                                        $(this).siblings(".eleTree-node-content-label").text(v).attr('title',v).show();
                                        $(this).remove();
                    ...
                    label.siblings(".eleTree-node-content-input").focus().select().off().on("blur",function() {
                        ...
                        layui.event.call(node, MOD_NAME, 'nodeEdit('+ _self.filter +')', {
                            ...
                            async: function() {
                                ...
                                // 修改dom
                                $(inpThis).siblings(".eleTree-node-content-label").text(val).attr('title', val).show();
                                $(inpThis).remove();
                            }
                        });
                        // 不是异步
                        if($.inArray("edit.async",options.contextmenuList)===-1){
                            ...
                            _self.reInitData(eleNode).currentData[options.request.name]=val;
                            // 修改dom
                            $(this).siblings(".eleTree-node-content-label").text(val).attr('title', val).show();
                            $(this).remove();
                        }
