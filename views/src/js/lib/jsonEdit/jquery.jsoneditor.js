...
(function( $ ) {
    ...
    function addNewValue(json,root) {

        if (isArray(json)) {
            json.push(json[json.length-1]?json[json.length-1]:null);
            return true;
        }
        /*if (isObject(json)) {

            var property=$(root.context.children[root.context.children.length-2]).find('.property').eq(0).val();
            var value=JSON.parse($(root.context.children[root.context.children.length-2]).find('.value').eq(0).val());

            var i = 1, newName = property;
