...
//			if (borderShown == "yes")  newTD.runtimeStyle.border = "1px dotted #BFBFBF";
		}
	}	
}
...
//			if (borderShown == "yes") newCell.runtimeStyle.border = "1px dotted #BFBFBF";
		}
   	}
}
...
function isTableSelected() {
	if (objSelectionForTable.Type == "Control") {
		var oControlRange = objSelectionForTable.Range;
		if (oControlRange(0).tagName.toUpperCase() == "TABLE") {
			selectedTable = objSelectionForTable.Range(0);
			return true;
		}	
	}
...
function isCursorInTableCell() {
	if (objSelectionForTable.Type != "Control") {
		var elem = objSelectionForTable.Range.parentElement()
