jsfw.WebUI.Toolbar = function (oDiv){
	...
	for (var i=0; i<oPopupMenu.root.children.length; i++){
		...
		oTab.className = (oItem.text=='')?'jsfw_ToolbarBtn_Split':'jsfw_ToolbarBtn';
		oTab.insertRow(0);
		oTab.rows[0].insertCell(0).className='jsfw_ToolbarBtn_left';
		oTab.rows[0].insertCell(1).className='jsfw_ToolbarBtn_center';
		oTab.rows[0].insertCell(2).className='jsfw_ToolbarBtn_right' + ((oItem.action=='')?'0':'');

		oTab.cellPadding=0; oTab.cellSpacing=0; oTab.border=0;

		oTab.title = oItem.text;
		if (oItem.icon.substr(0,4)=='$BG_'){
			var strTem=oItem.icon.substr(4);
			oTab.rows[0].cells[0].innerHTML = '<img src="'+jsfw.Path+'Themes/transparent.gif" style="background: url(\''+strTem.substr(strTem.indexOf('$')+1)+'\') no-repeat ' + strTem.substr(0,strTem.indexOf('$')).replace(/_/,'px ')+'px' + ';" width="16" height="16">';
		}else{
		...
		//设置按钮有效
		if (!oItem.enable) oTab.className = 'jsfw_ToolbarBtn_disabled';
		oItem.afterSetEnable = function (){
			var objs=oDiv.getElementsByTagName('table');
			for (var i=0; i<objs.length; i++){
				if (objs[i].obj==this){
					objs[i].className = this.enable?'jsfw_ToolbarBtn':'jsfw_ToolbarBtn_disabled';
					return;
				}
			}
		}
		...
		if (oItem.text!=''){
			oTab.onmouseover = function (){
				...
				//if (this.className=='jsfw_ToolbarBtn') this.className='jsfw_ToolbarBtn_Over';
			}
			oTab.onmouseout = function (){
				this.obj.mouseOver(false,'',jsfw.getAbsoluteLeft(this,true), jsfw.getAbsoluteTop(this,true)+this.offsetHeight-1);
				if (this.className=='jsfw_ToolbarBtn_Over') this.className='jsfw_ToolbarBtn';
			...
			if (oItem.isFolder && (oItem.action=='')){
				...
				oTab.rows[0].cells[0].onclick = oTab.rows[0].cells[1].onclick = function (){
					...
					objTem.obj.click();
					objTem.className='jsfw_ToolbarBtn_Over';
				}
			}
	...
	oDivMore.className='jsfw_Toolbar_More';
	oDivMore.onmouseover = function (){
		this.className='jsfw_Toolbar_MoreSel';
	}
	oDivMore.onmouseout = function (){
		this.className='jsfw_Toolbar_More';
	}
	...
	oPopupMenu.onMouseOverItem = function (aItem,bOver){
		...
		var objs=oDiv.getElementsByTagName('table');
		for (var i=0; i<objs.length; i++){
			if (objs[i].className=='jsfw_ToolbarBtn_Over') objs[i].className='jsfw_ToolbarBtn';
			if ((objs[i].obj==objTem) && aItem.enable) objs[i].className='jsfw_ToolbarBtn_Over';
		}
	}
	...
	oDiv.findItem = function(sID){ return oDiv.oPopupMenu.findItem(sID); }
...
jsfw.WebUI.Toolbar.TagName = 'div';
...
(
	function (){
		...
		link.rel = 'stylesheet';
		link.href = jsfw.Path + 'Themes/' + jsfw.Theme + '/Toolbar/css.css';
		link.type = 'text/css';
		link.media = 'all';
