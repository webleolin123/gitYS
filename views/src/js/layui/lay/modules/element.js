...
layui.define('jquery', function(exports){
  ...
  ,MOD_NAME = 'element', THIS = 'layui-this', SHOW = 'layui-show'
      ...
      , openIcon = {
          ...
          "width": "15px",
          "height": "3px",
          "vertical-align": "text-bottom", "left": "8px",
          // "padding-right": "5px",
          "top": "18px"
      }
      , closeIcon = {
          ...
          "width": "4px",
          "height": "20px",
          "vertical-align": "text-bottom", "left": "15px",
          "top": "10px"
      }
  ...
  Element.prototype.tabAdd = function(filter, options){
    ...
    var frm = document.getElementById(options.id);
/*    $(frm).load(function(){
      if(options.success){
    ...
    // if ($(frm)[0].attachEvent){
    //   $(frm)[0].attachEvent("onload", function(){ // IE  
    //     if(options.success){
    //       options.success(options);
    //     }
    //   });
    // } else {
    //   $(frm)[0].onload = function(){ // 非IE  
    //     if(options.success){
    //       options.success(options);
    //     }
    //   };
    ...
    ,collapse: function(){
      ...
      if(typeof parents.attr('lay-accordion') === 'string'){
        var show = parents.children('.layui-colla-item').children('.'+SHOW);
          // show.siblings('.layui-colla-title').children('.layui-colla-icon').html(openIcon);
          show.siblings('.layui-colla-title').children('.layui-colla-icon').css(openIcon);
        show.removeClass(SHOW);
      }

      elemCont[isNone ? 'addClass' : 'removeClass'](SHOW);
              icon.css(isNone ? closeIcon : openIcon);
      ...
      ,nav: function(){
        var TIME = 200, timer = {}, timerMore = {}, timeEnd = {}, follow = function(bar, nav, index){
          ...
          if(nav.hasClass(NAV_TREE)){
            bar.css({
              top: othis.offset().top - 100 - ($('.gnMenu').height() ? ($('.gnMenu').height() - 18) : 0)
            ...
            bar.css({
                left: othis.offset().left + parseFloat(othis.css('marginLeft'))
                , top: othis.offset().top + othis.height() - bar.height()
            });
            ...
            othis.on('mouseleave', function(){
              ...
              timeEnd[index] = setTimeout(function(){
                if(othis.hasClass(NAV_TREE)){
                  ...
                  bar.css({
                    width: 0
                      , left: bar.offset().left + bar.width() / 2
                    ,opacity: 0
                  });
                }
              }, TIME);
        ...
        $('.' + ELEM + elemFilter).each(function(){
          ...
          elemItem.each(function(){
            ...
            elemTitle.find('.layui-colla-icon').remove();
              elemTitle.append('<i class="layui-icon layui-colla-icon" ></i>');
              if (isNone) {

                  elemTitle.find('.layui-colla-icon').css(openIcon)
              } else {
                  elemTitle.find('.layui-colla-icon').css(closeIcon)
              }
              // elemTitle.append('<i class="layui-icon layui-colla-icon">' + (isNone ? openIcon : closeIcon) + '</i>');
