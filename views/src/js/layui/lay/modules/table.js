        ...
        ,error: function(e, m){
          if(e.status===403){
            layer.msg('请重新登入', {icon: 5,time:3000},function () {
              ...
              // window.top
              if(window.top){
                window.top.location.replace(window.top.location.origin + '/login.html');
              }else{
            ...
            try {
              that.layMain.html('<div class="' + NONE + '">数据接口请求异常：' + JSON.parse(e.responseText).resultMessage + '</div>');
            } catch (e) {
              that.layMain.html('<div class="' + NONE + '">数据接口请求异常：' + e + '</div>');
            }
          }
  ...
  Class.prototype.renderTotal = function(data){
    ...
    layui.each(data, function(i1, item1){
      ...
      that.eachCols(function(i3, item3){
        ...
        if(item3.totalRow){
          if (item3.initVal) {
            totalNums[field] = parseInt((totalNums[field] || 0) + (parseFloat(content) || 0));
          } else {
    ...
    that.eachCols(function(i3, item3){
      ...
      var td = ['<td data-field="'+ field +'" data-key="'+ options.index + '-'+ item3.key +'" '+ function(){
        ...
        ,'<div class="layui-table-cell laytable-cell-'+ function(){ //返回对应的CSS类标识
          ...
          if (item3.initVal) {
            return item3.totalRow ? (parseInt(totalNums[field]) || text) : text;
          } else {
